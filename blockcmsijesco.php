...
require_once _PS_MODULE_DIR_.'blockcmsijesco/classes/InfoBlockJesco.php';

class Blockcmsijesco extends Module
{
	public $html = '';

	public function __construct()
	{
		$this->name = 'blockcmsijesco';
		$this->tab = 'front_office_features';
		$this->version = '1.0.0';
		$this->author = 'Lord Jesco';
		$this->bootstrap = true;
	...
	public function installDB()
	{
		...
		$return &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'jesco_informacion` (
		...
		$return &= Db::getInstance()->execute('
				CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'jesco_informacion_lang` (
	...
	public function uninstallDB($drop_table = true)
	{
		$ret = true;
		if($drop_table)
			$ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'jesco_informacion`') && Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'jesco_informacion_lang`');
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('saveblockcmsijesco'))
		{
		...
		elseif (Tools::isSubmit('updateblockcmsijesco') || Tools::isSubmit('addblockcmsijesco'))
		{
			$this->html .= $this->renderForm();
			return $this->html;
		}
		else if (Tools::isSubmit('deleteblockcmsijesco'))
		{
			$info = new InfoBlockJesco((int)$id_info);
			$info->delete();
			$this->_clearCache('blockcmsijesco.tpl');
	...
	protected function renderForm()
	{
		...
		$helper->module = $this;
		$helper->name_controller = 'blockcmsijesco';
		...
		$helper->submit_action = 'saveblockcmsijesco';
	...
	protected function getListContent($id_lang = null)
	{
		...
		$sql = 'SELECT r.`id_info`, rl.`text`, s.`name` as shop_name
			FROM `'._DB_PREFIX_.'jesco_informacion` r
			LEFT JOIN `'._DB_PREFIX_.'jesco_informacion_lang` rl ON (r.`id_info` = rl.`id_info`)
			LEFT JOIN `'._DB_PREFIX_.'jesco_informacion_shop` s ON (r.`id_shop` = s.`id_shop`)
	...
	public function hookHome($params)
	{
		...
		if (!$this->isCached('blockcmsijesco.tpl', $this->getCacheId()))
		{
			$infos = $this->getInfos($this->context->language->id, $this->context->shop->id);
			$this->context->smarty->assign(array('jesco_informacion' => $infos, 'nbblocks' => count($infos)));
		}

		return $this->display(__FILE__, 'blockcmsijesco.tpl', $this->getCacheId());
	}
